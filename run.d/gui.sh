...
su -l www-data -c "$CMD --src $CFGD/schema.yaml --src $CFGD/openaire.yaml --src $CFGD/drupal.yaml --src $CFGD/local.yaml --src $CFGD/repo.yaml --srcPath '$.metadataManagment.nonRelationProperties' --targetPath '$.metadataManagment.nonRelationProperties' $CFGD/config-gui.yaml"
